...
| Path | Purpose |
|---|---|
| `lib/destructive-policy.sh` | Shared shell policy for destructive command detection and self-protection checks. |
...
| `tests/` | Harness-specific tests for Claude, Codex, and Pi hook behavior. |
...
- Claude, Codex, and Pi share the same destructive-command policy.
...
These guards are heuristic friction. They can catch common destructive shell commands and self-modification attempts, but they do not replace sandboxing, backups, human review, or a trusted hook configuration. Codex command hooks also require trust review through the harness UI according to the Codex hook docs.

If your workflow has additional risky commands, ask an agent to help extend `lib/destructive-policy.sh` and the tests for those tools before installing the guard. Review the proposed patterns because a custom guard can have false positives and false negatives.
...
- The default destructive guard is intentionally generic. Use agent assistance to add reviewed, tested checks for project-specific tools before relying on it in that workflow.
