...
name: web-discovery
title: Web Discovery
description: Adds external web/code/docs discovery and retrieval whenever web-available context could improve correctness, freshness, provenance, examples, docs, API usage, product/pricing facts, source verification, or research quality. Use unless the user explicitly forbids web/search/network access or requests local-only/offline work. Routes ordinary lookup to Ketch, fuzzy semantic discovery to Brave, and flags unavailable or browser-required tiers explicitly.
compatibility: Requires ketch on PATH.
metadata:
  version: 0.2.0
  policy-version: v0
tags:
...
- Default cheap discovery: `ketch search` for web, `ketch code` for code, `ketch docs` for docs.
- Default retrieval/extraction: `ketch scrape <url>`.
...
- Use Brave Search only for fuzzy/semantic/similar-source discovery where keyword search is likely weak.
