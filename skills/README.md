...
A skill is a reusable instruction bundle for an agent. The main file is usually `SKILL.md`; extra examples or scripts may live beside it. A harness is the program that runs the model and tools, such as Codex, Claude Code, or Pi.
...
- `web-discovery`: web/code/docs research. It needs network permission plus Ketch or an equivalent search-plus-retrieval setup. Search results are leads; retrieved source text is evidence.
